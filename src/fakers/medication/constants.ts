import type {ICoding, IMedication, IMedication_Batch, IMedication_Ingredient} from "@ahryman40k/ts-fhir-types/lib/R4";
...
const MEDICATION_INGREDIENT_ACTIVE_PROBABILITY = .25
...
const rxNormCodings: CodingSeed[] = [
    {
        "code": "993891",
        "display": "acetaminophen 300 MG / codeine phosphate 60 MG [Tylenol with Codeine]"
    }, {
        "code": "993770",
...
const ndcCodings: CodingSeed[] = [
    ...
    {
        "code": "55758-012-05",
        "display": "5 PACKET in 1 BOX (55758-012-05) / 1 GRANULE, FOR SOLUTION in 1 PACKET"
    },
    {
        "code": " 0169-4130-01",
...
const createMedicationCoding = (): ICoding => faker.helpers.arrayElement([createRxNormCoding(), createNDCCoding()])
...
export const  createMedicationBatch = (): IMedication_Batch => {
    return {
        lotNumber: faker.string.numeric({allowLeadingZeros: true, length: {min: 2, max: 4}}),
        expirationDate: faker.helpers.arrayElement([faker.date.future(), faker.date.past({years: 10})]).toISOString()
    }
}
