...
const clientFhirPersonId = "<YOUR_CLIENT_FHIR_PERSON_ID>"; // Replace with your actual client FHIR Person ID

const clientFhirPatientId = "<YOUR_CLIENT_FHIR_PATIENT_ID>"; // Replace with your actual client FHIR Patient ID
...
async function createResourceIfNotExists(reference, env, accessToken) {
  ...
  const exists = await checkResourceExists(parsedRef.resourceType, parsedRef.resourceId, env, accessToken);
...
async function makeEncounterResolvableInMedDispense() {
    try {
        ...
        if (response.status === 200 && response.data && response.data.entry) {
            const medicationDispenses = response.data.entry.map(entry => entry.resource);
            console.log(`Retrieved ${medicationDispenses.length} medication dispenses`);
            ...
            medicationDispenses.forEach(medicationDispense => {
                ...
                if (medicationDispense.context) {
                    ...
                    if (medicationDispense.context.reference && 
            ...
            console.log(`Found ${encounterReferences.size} unique encounter references`);
            ...
            console.error('Failed to retrieve medication dispenses or no entries found');
...
if (require.main === module) {
    ...
    makeEncounterResolvableInMedDispense().then((result) => {
        if (result && result.encounterReferences) {
            console.log(`Summary: Processed ${result.encounterReferences.length} unique encounter references`);
            if (result.createdEncounters) {
                console.log(`Created ${result.createdEncounters} encounters that were missing`);
            }
        } else {
        ...
        console.error("Error making encounter resolvable in MedicationDispense:", error);
    });
}
