 ...
 */
async function createResourceIfNotExists(reference) {
  ...
  try {
    ...
    const url = `${config.useProaServer ? PROA_FHIR_BASE_URL : FHIR_BASE_URL}/${
      parsedRef.resourceType
    }/${parsedRef.resourceId}`;
...
async function makeEncounterResolvableInMedDispense(resource) {
  try {
    const url = `${
    ...
    if (response.status === 200 && response.data && response.data.entry) {
      ...
      const results = await Promise.all(
        encounterArray.map((reference) => createResourceIfNotExists(reference))
      );
          ...
          .map(async (medicationDispense) => {
            ...
            if (foundEncounter) {
              ...
              if (uuidIdentifier) {
                ...
                const url = `${
                  config.useProaServer ? PROA_FHIR_BASE_URL : FHIR_BASE_URL
                }/MedicationDispense/${updatedDispense.id}`;
